...
============================================================
 ...
 All four use the identical pipeline:
   ...
   Amendment 3 — Naive baselines (Random Walk + Persistence)
   Amendment 4 — Data leakage controls

 FIX (this version):
   1. Seed control added at the top, for consistency with
      crypto_eda.py, crypto_models_v3.py, and crypto_charts_v2.py.
   2. build_sequences_for_fold()'s demo sequence-shape check (Step 7)
      now carries the last SEQ_LEN days of training data forward as
      history for the test window, so the printed X_test shape shows
      the full test_days sequences (e.g. 90), matching the corrected
      crypto_models_v3.py and crypto_charts_v2.py, instead of the
      shorter (test_days - SEQ_LEN) shape from windowing loss. This
      function's output isn't saved anywhere in this script — it's a
      sanity-check printout only — so this fix affects console output
      consistency, not any saved results.
   3. Replaced deprecated fillna(method="ffill") with .ffill(), since
      the method= argument is removed in pandas 3.0.
   4. Fixed stale "crypto_models_v2.py" reference in the final printed
      instructions to say crypto_models_v3.py.
============================================================
"""
...
def download_ohlcv(ticker: str, start: str, end: str) -> pd.DataFrame:
    ...
    if len(df) < TRAIN_WINDOW + TEST_WINDOW + SEQ_LEN:
        ...
        print(f"    OK {df.shape[0]} rows | {df.index[0].date()} -> {df.index[-1].date()}")
    return df
...
def download_fear_greed(limit: int = 2000) -> pd.DataFrame:
    ...
    try:
        ...
        df.sort_index(inplace=True)
        print(f"  OK {df.shape[0]} daily records | "
...
def engineer_features(df: pd.DataFrame, fg: pd.DataFrame,
    ...
    if not fg.empty:
        ...
        d["FearGreed_Score"] = d["FearGreed_Score"].ffill()  # FIX: was fillna(method="ffill")
    else:
...
print("\n" + "=" * 65)
print("STEP 4: Computing naive baselines")
print("=" * 65)
baselines = {}
...
# ── 7. LEAKAGE-FREE SEQUENCE BUILDER (FIXED) ─────────────────────────────────
...
def build_sequences_for_fold(df, train_idx, test_idx, seq_len=SEQ_LEN):
    ...
    FIX: the last `seq_len` rows of scaled TRAINING data are prepended
    to the test slice before windowing, purely as historical context,
    so X_test/y_test contain one sequence per test day (matching
    test_days), instead of losing the first `seq_len` test days to the
    windowing operation. This matches the same fix applied in
    crypto_models_v3.py and crypto_charts_v2.py.
    """
    ...
    test_scaled_with_history = np.vstack([train_scaled[-seq_len:], test_scaled])

    target_scaler = MinMaxScaler(feature_range=(0, 1))
    train_targets = target_scaler.fit_transform(tr[["Target"]])
    test_targets  = target_scaler.transform(te[["Target"]])
    test_targets_with_history = np.vstack([train_targets[-seq_len:], test_targets])
    ...
    X_te, y_te = make_sequences(test_scaled_with_history, test_targets_with_history, seq_len)
...
print(f"  (X_test should now have {len(s0_te)} sequences, matching the "
      f"{len(s0_te)}-day test window, not {len(s0_te) - SEQ_LEN})")
...
for name, df in processed.items():
    ...
    print(f"  OK Saved: {fname} ({df.shape[0]} rows x {df.shape[1]} cols)")
...
plt.show()
print("  OK Saved: all_coins_dataset_overview.png")
...
plt.show()
print("  OK Saved: all_coins_price_comparison.png")
