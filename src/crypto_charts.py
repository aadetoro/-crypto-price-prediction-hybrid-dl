...
============================================================
 ...
 Run AFTER crypto_dataset_pipeline.py and crypto_models_v3.py

 FIX (this version):
   1. Now loads all 4 coins (Bitcoin, Ethereum, Binance Coin, Litecoin),
      not just Bitcoin and Ethereum.
   2. build_sequences() now carries the last SEQ_LEN days of TRAINING
      data forward as history for the test window, so every deep
      learning model produces a prediction for all 90 test days —
      matching ARIMA's coverage and matching the 90-day-per-fold test
      RMSE reported in crypto_models_v3.py. Previously, deep learning
      models only produced 30 test predictions (90 - 60 lost to
      windowing), so the chart's blue "test" line and its printed
      Test RMSE covered a shorter, different sample than the official
      results table.
   3. Seed control added for reproducibility, consistent with the
...
for name, fname in COIN_FILES.items():
    if os.path.exists(fname):
        ...
        datasets[name] = df
        print(f"  OK {name:<14} {df.shape[0]} rows")
    else:
...
if len(datasets) < len(COIN_FILES):
    ...
    print("  *** Colab runtime before continuing. ***\n")
...
def build_sequences(df, train_idx, test_idx, seq_len=SEQ_LEN):
    ...
    FIX: the last `seq_len` rows of TRAINING data are prepended to the
    test slice before windowing, purely as historical context, so every
    one of the `test_days` days in the test fold gets a prediction
    (matching ARIMA's coverage) instead of losing the first `seq_len`
    test days to the windowing operation. The scaler itself is still
    fit on training data only — no leakage is introduced.
    """
    ...
    assert len(X_te) == len(test_idx), (
        f"Expected {len(test_idx)} test predictions, got {len(X_te)} — "
...
if __name__ == "__main__":
    ...
    print(f"  Coins processed: {', '.join(datasets.keys())} ({len(datasets)}/4)")
    print("="*60)
  ...
  To download in Colab, run:
    ...
    "".join(f'\n    files.download("{f}")' for f in chart_files) + """
""")
    print("="*60)
